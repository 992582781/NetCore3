...
namespace Redis
{
    namespace Redis_Helper
    {
        ...
        public class RedisHelper
        {
            ...
            {
                key = AddSysCustomKey(key);
                string values = Do(db => db.StringGet(key));
                if (values == null)
                {
                    return default(T);
                }
            ...
            {
                key = AddSysCustomKey(key);
                string values = await Do(db => db.StringGetAsync(key, CommandFlags.PreferSlave));
                //return ConvertObj<T>(values);
                if (values == null)
                {
                    return default(T);
            ...
            /// <param name="dataKeys"></param>
            /// <returns></returns>
            public bool HashDelete(string key)
            {
                key = AddSysCustomKey(key);
            ...
            public T HashGet<T>(string key, string dataKey)
            {
                ...
                return Do(db =>
                {
                    string values = db.HashGet(key, dataKey);
                    if (values == null)
                    {
                        return default(T);
                    }
            ...
            {
                key = AddSysCustomKey(key);
                string values = await Do(db => db.HashGetAsync(key, dataKey));
                if (values == null)
                {
                    return default(T);
                }
            ...
            /// <summary>
            /// 模糊查询所有相识键值,此方法尽可能不用，会降低redis效率
            /// *：通配任意多个字符?：通配单个字符[]：通配括号内的某一个字符
            /// </summary>
            /// <param name="pattern">参数</param>
            /// <returns></returns>
            public List<string> ScriptEvaluate(string pattern)
            {
                ...
                var values = Do(redis => redis.ScriptEvaluate(LuaScript.Prepare(
                            //Redis的keys模糊查询：
                            " local res = redis.call('KEYS', @keypattern) " +
                            " return res "), new { @keypattern = pattern }));
                List<string> preSult = ((string[])values).ToList();//将返回的结果集转为数组
                return preSult;
            }
            ...
            private T Do<T>(Func<IDatabase, T> func)
            {
                ...
                {
                    database = _conn.GetDatabase(DbNum);
                    //database = ChangeMaster.mex.GetDatabase(DbNum);
            ...
            private string ConvertJson<T>(T value)
            {
                ...
                string result = value is string ? value.ToString() : JsonConvert.SerializeObject(value, Formatting.Indented, timeFormat);
                return result;
            }
            ...
            private T ConvertObj<T>(HashEntry value)
            {
                if (typeof(T).Name.Equals(typeof(string).Name))
                {
